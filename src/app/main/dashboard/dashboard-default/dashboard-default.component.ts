...
export class DashboardDefaultComponent implements OnInit {
  ...
  obtenerDiaActual(diaSeteado?){
    ...
    if(!diaSeteado){
      ...
      if(mes != this.mesSeleccionado){
        ...
        this.toast('Error: Debe seleccionar el mes correcto.', 5000);
        return;
      }
      ...
      this.discrimineDataChart('dia');
      // this.mesSeleccionado = diaSeteado;
    }
  }

  discrimineDataChart(rango){
    debugger
    let key = Object.keys(this.data.tablero)[0];
      ...
      Object.keys(dia[i]).forEach(e => {
        if(e != 'PDF' && e != 'CAE' && e != 'MAIL' && e != 'horas'){
          if(dia[i].e){
            ...
            this.chartCantidadOTRO.push({fecha: fecha, data: dia[i].e.tiempoPromedio})
          }else{
  ...
  createChartStatics(){
    ...
    console.log(this.chartVelocidadOTRO);
